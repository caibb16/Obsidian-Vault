...
> 截至 2026-08-03，已完成 **SRGPO 框架接入** + **env-verifiable 步骤奖励**。
...
在 ActiveVLN（ICRA 2026）的稀疏 GRPO 上**叠加一层 SRGPO 的过程奖励**：env 端每个动作执行后给出一个 `step_reward ∈ {0,1,2}`（距离减少 1cm / 首次进入 oracle 区域），rollout 端聚合成 `raw_step_rewards`，trainer 端通过 `episode_adv + ω · step_adv` 注入 advantage。可在 `srgpo_step_advantage_w=0` 时退化为纯 GRPO，便于消融。
...
## 5. 阶段 2：SeeNav-Agent 项目 env-verifiable 步骤奖励
