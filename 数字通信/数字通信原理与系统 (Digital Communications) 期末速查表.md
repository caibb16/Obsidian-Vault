...
**4. 一些定义**
- **Q函数**：$Q(x) = Pr[N(0,1) \gt x]= \frac{1}{\sqrt{2\pi}} \int_{-\infty}^x e^{-u^2/2} du$
- **自相关函数**：$R_{t_1, t_2}(\tau) = E[X(t_1)X^*(t_2)]$  
- **互相关函数**：$R_{t_1, t_2}(\tau) = E[X(t_1)Y^*(t_2)]$
- **功率谱密度**：$S_X(f) = \lim\limits_{T \to \infty} \frac{E|F_T(f)|^2}{T} =\mathcal{F}\left\{R_X(\tau)\right\}$ (WSS)
